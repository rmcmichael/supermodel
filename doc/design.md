...
1. **Database** — model registry hooks, `transaction()`, schema ensure on first connection / late registration; unit tests for registry, transactions, and schema timing
2. **Column types** — DDL fragments and to/from SQL codecs; unit tests for each column type’s DDL and value conversion
